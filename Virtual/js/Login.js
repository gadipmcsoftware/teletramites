    ...
    try {
        ...
        $('#Btn_login').click(function() { 
                ...
                if(row.length>0){
                    ...
                    sessionStorage.setItem('PRE_REPSIMULA',row[0][7]);
                    sessionStorage.setItem('PRE_SIMULA',row[0][8]);
                    ...
                    buscarLoginProfe(usuario,clave);
                     if(rowProfe.length>0){
                           ...
                           if(cam_clave==1){
                               window.location.href = "http://172.23.25.6/Virtual/CambioClave.php";
                           }
                           else{
                                window.location.href = "http://172.23.25.6/Virtual/index.php";
                           }
...
//                        if(ind_form){
					...
					async: false,
					cache: false,
					url: "http://172.23.25.6/Virtual/logica/ObtenerLogin.php",
					data: parametros,
   ...
   function buscarLoginProfe(usuario,clave){
    var ex=0; 
     try {
					...
					async: false,
					cache: false,
					url: "http://172.23.25.6/Virtual/logica/ObtenerLoginProfe.php",
					data: parametros,
					success: function(response){                                           
						 if(JSON.parse(response.ind)){
                                                               ex=response.ind;

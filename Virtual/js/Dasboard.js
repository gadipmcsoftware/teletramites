                                          ...
                                          if(rowMensaje.length>0){  
                                                  for(var i=0; i<rowMensaje.length; i++){
                                                       ...
                                                       if(rowMensaje[i][5]==0){
                                                           ...
                                                           sessionStorage.setItem('Notificacion',rowDataNotif[11]);
                                                           window.location.href = "http://172.23.25.6/Virtual/FrmRegistro.php?area=0";
                                                       }
                                                  }     
                                          }
                                          else{
...
//                               
                                    $('#Form_Recorrido').modal('show');
                                     ...
                                     if($(this).attr('id')=="Btn_arch"){
                                        var reply = window.prompt("Razon por la que Archiva el Tramite?");
                                        if(reply==undefined)
                                        {
...
//                                                alert(rowDataNotif[11]+" "+rowDataNotif[0]+" "+reply)
                                                ArchivaTramite(rowDataNotif[11],rowDataNotif[0],reply,2);
                                                 window.location.href ="http://172.23.25.6/Virtual/index.php";
                                        }
                                     }
                                }
                         ...
                         for(var i=0; i<Notificaciones.length; i++){
                              tbl_tramite.row.add([
                                                     ...
                                                     "<a id='Btn_resp' href='#' class='btn btn-success btn-circle'><i class='far fa-check-circle'></i>",
                                                      ...
                                                      "<a id='Btn_arch' href='#' class='btn btn-warning btn-circle'><i class='fas fa-file-invoice'></i>",
                                                     Notificaciones[i][7]
...
function DatosTramites(id,clave){
   ...
   var ex=0; 
     try {
					...
					async: false,
					cache: false,
					url: "http://172.23.25.6/Virtual/logica/TraerTramite.php",
					data: parametros,
					success: function(response){                                           
						 if(JSON.parse(response.ind)){
                                                               ex=response.ind; 
...
function DatosEstadistica(id,usr){
   ...
   var ex=0; 
     try {
					...
					async: false,
					cache: false,
					url: "http://172.23.25.6/Virtual/logica/ConsBarBan.php",
					data: parametros,
					success: function(response){                                           
						 if(JSON.parse(response.ind)){
                                                               ex=response.ind; 
...
function TraeMensaje(id){
   ...
   var ex=0; 
     try {
					...
					async: false,
					cache: false,
					url: "http://172.23.25.6/Virtual/logica/TraeMensajes.php",
					data: parametros,
					success: function(response){                                           
						 if(JSON.parse(response.ind)){
                                                               ex=response.ind; 
...
function DatosTramitesPadre(id,clave){
   ...
   var ex=0; 
     try {
					...
					async: false,
					cache: false,
					url: "http://172.23.25.6/Virtual/logica/TrearTramitePadre.php",
					data: parametros,
					success: function(response){                                           
						 if(JSON.parse(response.ind)){
                                                               ex=response.ind; 

...
if( $result1 === false) {
    if( ($errors = sqlsrv_errors() ) != null)  
          ...
          echo json_encode($json_data);
                ...
                {                                                                                                                            
                      array_push($consulta, array($row["NUM_TRAMI"],$row["CED_PROFE"],$row["NOM_PROFE"],$row["DESC_TIP_TRA"],$row["NOMBRE"],$row["DESC_ESTA_TRA"],$row["DESCRIPTION"],$row["CLAVE_CATASTRAL"],$row["OBSE_TRAMITE"]));

...
$query1 = "{call SP_TRAENOTIFICACIONES(?,?)}";
...
if( $result1 === false) {
    if( ($errors = sqlsrv_errors() ) != null)  
          ...
          echo json_encode($json_data);
                ...
                {                                                                                                                            
                      array_push($consulta, array($row["NUM_TRAMI"],$row["CLAVE_CATASTRAL"],$row["NOM_PROFE"],$row["DESCRIPTION"],$row["DESC_ESTA_TRA"],$row["DESC_NOTIFICA"],$row["FECHA_NOTIFI"],$row["COD_NOTIFI"],$row["NOTIFI"],$row["PROPIETARIO"]));

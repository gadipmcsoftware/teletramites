...
$query1 = "{call SP_TraeProfecional(?)}";
...
if( $result1 === false) {
    if( ($errors = sqlsrv_errors() ) != null)  
          ...
          echo json_encode($json_data);
                ...
                {                                                                                                                            
                      array_push($consulta, array($row["CED_PROFE"],$row["NOM_PROFE"],$row["TELEF_PROFE"],$row["EMAIL_PROFE"],$row["DIREC_PROFE"],$row["NOTI_EMAIL"],$row["PER_TRAM"]));
